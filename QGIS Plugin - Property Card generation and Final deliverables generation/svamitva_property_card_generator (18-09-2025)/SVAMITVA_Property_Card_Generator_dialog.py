...
from qgis.PyQt.QtGui import QColor, QFont, QIcon, QPixmap   # ✅ added QIcon, QPixmap
...
class SVAMITVAPropertyCardGeneratorDialog(QDialog, FORM_CLASS):
    def __init__(self, iface, parent=None):
            ...
            for fname in os.listdir(fonts_dir):
                if fname.lower().endswith(".ttf"):
                    ...
                    if font_id != -1:
                        ...
                        self.logTextEdit.append(f"✅ Loaded font: {loaded}")
                    else:
                        self.logTextEdit.append(f"⚠️ Could not load font: {fname}")
        ...
        # ✅ Browse button for photos folder
        self.photosBrowseButton.clicked.connect(self.select_photos_folder)
        
        # ✅ Browse button for Attributes Excel folder
    ...
    def load_qpt_template(self, qpt_path, parcel_layer):
        ...
        if not os.path.exists(qpt_path):
            self.logTextEdit.append(f"⚠️ QPT template not found: {qpt_path}")
            return
        ...
        try:
            ...
            if not doc.setContent(template_content):
                self.logTextEdit.append("❌ Invalid QPT template XML.")
                return
            ...
            self.logTextEdit.append("✅ Property Card template loaded successfully.")
            ...
            if hasattr(atlas, "setSingleFileExportMode"):
                ...
                try:
                    ...
                    self.logTextEdit.append("⚠️ Atlas single-file export setting skipped (not supported in this QGIS).")

            # Output filename expression
            atlas.setFilenameExpression("'Property Card_' || @atlas_featurenumber")

            # ✅ Now configure "PPM location Coordinates (Table)" table
            ...
            if table_frame:
                ...
                if inaccessible_point:
                    ...
                    self.logTextEdit.append("✅ Table 'PPM location Coordinates (Table)' updated with Latitude/ Longitude only.")
                else:
                    self.logTextEdit.append("⚠️ Inaccessible Point layer not found in project.")
            else:
                self.logTextEdit.append("⚠️ No table item named 'PPM location Coordinates (Table)' in template.")
                

            # ✅ Delay opening the layout designer
            QTimer.singleShot(1000, lambda: self.iface.openLayoutDesigner(layout))
                
            # ✅ Open layout in designer and preview first feature
            # self.iface.openLayoutDesigner(layout)
            
            self.logTextEdit.append("✅ Property Card template loaded & Atlas configured successfully.")
            
        except Exception as e:
            self.logTextEdit.append(f"❌ Error loading QPT template or configuring Atlas: {e}")
    ...
    def generate_ppms(self):
        ...
        try:
            ...
            self.logTextEdit.append(f"📂 Output directory created: {out_dir}")
            self.logTextEdit.append(f"📂 Shapefiles Output directory created: {out_dir_shapefiles}")
            ...
            if orig_parcel_layer is None:
                self.logTextEdit.append("❌ No parcel layer selected.")
                return
            ...
            try:
                ...
                if _error == QgsVectorFileWriter.NoError:
                    self.logTextEdit.append(f"✅ Parcel layer saved as: {parcel_out_path}")
                else:
                    self.logTextEdit.append(f"⚠️ Error saving parcel layer: {_error}, {_error_msg}")
            except Exception as e:
                self.logTextEdit.append(f"⚠️ Exception saving parcel layer: {e}")
                return
            ...
            if not excel_path:
                QMessageBox.warning(self, "Missing Input", "⚠️ Please select an Excel file.")
                return
            ...
            try:
                ...
                self.logTextEdit.append(f"✅ Excel copied to: {excel_copy_path}")
            except Exception as e:
                self.logTextEdit.append(f"⚠️ Error copying Excel: {e}")
                return
            ...
            if not owners_layer.isValid():
                self.logTextEdit.append("❌ Could not load Excel into QGIS. Check sheet name.")
                return

            QgsProject.instance().addMapLayer(owners_layer)
            self.logTextEdit.append(f"✅ Excel loaded: {owners_layer.name()}")
            ...
            try:
                self.logTextEdit.append("🔗 Joining parcel layer with Excel table...")
                ...
                if parcel_layer.isValid():
                    ...
                    self.logTextEdit.append(f"✅ Joined layer created: {join_out_path}")
                else:
                    self.logTextEdit.append("❌ Joined layer invalid after processing.")
            except Exception as e:
                self.logTextEdit.append(f"⚠️ Error during join: {e}")
            ...
            if os.path.exists(photos_source):
                ...
                self.logTextEdit.append(f"📂 Photos copied to: {photos_dest}")
            else:
                self.logTextEdit.append("⚠️ No source photos folder found.")
            ...
            def run_and_save(alg, params, out_name):
                ...
                if not layer.isValid():
                    self.logTextEdit.append(f"❌ Failed to create {out_name}")
                else:
                    QgsProject.instance().addMapLayer(layer)
                    self.logTextEdit.append(f"✅ {out_name} saved: {out_path}")
                return layer

            # 3) Boundary
            self.logTextEdit.append("1/12 → Creating boundary...")
            ...
            self.logTextEdit.append("2/12 → Exploding lines from boundary...")
            ...
            self.logTextEdit.append("3/12 → Extracting vertices from boundary...")
            ...
            self.logTextEdit.append("4/12 → Applying symbology and labeling to parcel layer...")
            try:
                ...
                # tf_a.setSize(15)   # ✅ Force label size 15 pt
                ...
                font.setUnderline(True)   # ✅ underline applied here

                tf_b.setFont(font)
                # tf_b.setSize(15)   # ✅ Force label size 15 pt
                tf_b.setColor(QColor('black'))
                
                pal_b.setFormat(tf_b)

                # ✅ Enable data-defined size expression
                # If PPM < 10 → 10
                # If PPM < 100 → 8
                # If PPM < 1000 → 6.2
                # Else → 4.2
                # ✅ Expression for size based on PPM
                size_expr = """
                CASE
                    WHEN "PPM" < 10 THEN 10
                    WHEN "PPM" < 100 THEN 8
                ...
                self.logTextEdit.append(f"⚠️ Symbology/labeling step raised an error: {e}")
            ...
            self.logTextEdit.append("5/12 → Creating multi-ring buffer (distance 2, rings 1) around vertices...")
            ...
            self.logTextEdit.append("6/12 → Clipping exploded lines by buffer...")
            ...
            self.logTextEdit.append("7/12 → Exploding lines from clip...")
            ...
            self.logTextEdit.append("8/12 → Applying symbology to explode lines (boundary-derived)...")
            try:
                ...
                self.logTextEdit.append(f"⚠️ Explode lines symbology error: {e}")

            self.progressBar.setValue(70)

            # 11) Add new field "length" to explode_lines and populate with $length (Decimal number, length=10, precision=2)
            self.logTextEdit.append("9/12 → Adding 'length' field to explode lines and populating values...")
            try:
                if explode_lines.dataProvider().capabilities() & QgsVectorDataProvider.AddAttributes:
                    ...
                    self.logTextEdit.append("⚠️ Cannot add field to explode_lines (provider doesn't support add).")
            except Exception as e:
                self.logTextEdit.append(f"⚠️ Error adding/calculating length: {e}")

            self.progressBar.setValue(75)

            # 12) Label explode_lines with length (Single Labels), with size expression and placement.
            self.logTextEdit.append("10/12 → Applying labeling to explode lines (show 'length')...")
            try:
                ...
                self.logTextEdit.append(f"⚠️ Labeling explode lines error: {e}")

            # 13) Clip explode lines symbology rule-based
            self.logTextEdit.append("11/12 → Applying symbology to clip explode lines...")
            try:
                ...
                self.logTextEdit.append(f"⚠️ Clip explode lines symbology error: {e}")

            self.progressBar.setValue(80)

            # 14) In vertices: add point_ID, Easting_X, Northing_Y fields and populate
            self.logTextEdit.append("12/12 → Adding point_ID, Easting_X, Northing_Y fields to vertices and populating...")
            try:
                if vertices.dataProvider().capabilities() & QgsVectorDataProvider.AddAttributes:
                    ...
                    self.logTextEdit.append("⚠️ Cannot add fields to vertices (provider doesn't support add).")
            except Exception as e:
                self.logTextEdit.append(f"⚠️ Error adding/populating vertex fields: {e}")
            ...
            try:
                ...
                if _error == QgsVectorFileWriter.NoError:
                    self.logTextEdit.append(f"✅ No-duplicate vertices layer created and saved: {no_dup_path}")
                    ...
                    self.logTextEdit.append("🎨 Symbology applied to no-duplicate vertices layer.")
                    ...
                    self.logTextEdit.append(f"⚠️ Error saving no-duplicate vertices layer: {_error}, {_error_message}")
            ...
            try:
                ...
                self.logTextEdit.append(f"⚠️ Error applying symbology to no-dup vertices: {e}")
            ...
            try:
                ...
                self.logTextEdit.append(f"⚠️ Hide intermediate layers error: {e}")
            ...
            try:
                ...
                self.logTextEdit.append(f"⚠️ Error reordering layers: {e}")
            ...
            #     self.logTextEdit.append(f"✅ Center point created and saved: {center_point_path}")
            ...
            #         # ✅ safer filter: only apply if "PPM" exists
            ...
            #         self.logTextEdit.append("🎨 Center point symbology applied.")

            #     except Exception as e_cp_field:
            #         self.logTextEdit.append(f"⚠️ Error adding/populating center point fields: {e_cp_field}")

            # except Exception as e:
            #     self.logTextEdit.append(f"⚠️ Point on surface error: {e}")
            ...
            try:
                ...
                self.logTextEdit.append(f"✅ PoI created and saved: {poi_path}")
                ...
                self.logTextEdit.append("🎨 PoI symbology applied.")

            except Exception as e:
                self.logTextEdit.append(f"⚠️ Error generating PoI: {e}")
            ...
            try:
                ...
                self.logTextEdit.append(f"💾 QGIS project saved: {project_filename}")
            except Exception as e:
                self.logTextEdit.append(f"⚠️ Error saving QGIS project: {e}")
            ...
            for feature in parcel_layer.getFeatures():
                ...
                for c in candidates:
                    if os.path.exists(c):
                        # ✅ store as relative path (relative to project directory)
                        rel_path = os.path.relpath(c, project_dir).replace("\\", "/")
                        photo_file = rel_path
                        break

                # If still no photo found → use Blank_Photo.png
                if not photo_file:
            ...
            self.logTextEdit.append("✅ Photo paths added to parcel layer successfully.")
            ...
            self.logTextEdit.append(f"👍 QGIS Template saved: PropertyCard_SINGLE_SVAMITVA.qpt")
            ...
            try:
                ...
                project_filename = os.path.join(out_dir, f"{village}_{lgd_code}_PropertyCard.qgz")
                project.write(project_filename)
                self.logTextEdit.append(f"💾 QGIS project re-saved with layout: {project_filename}")
            except Exception as e:
                self.logTextEdit.append(f"⚠️ Error re-saving QGIS project: {e}")
                
            self.progressBar.setValue(100)
            self.logTextEdit.append("✅ PPM generation pipeline completed.")

        except Exception as e:
            self.logTextEdit.append(f"❌ Unexpected error: {e}")
            raise